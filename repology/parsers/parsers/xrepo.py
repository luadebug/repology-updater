...
def _parse_lua_string(s: str) -> str:
    ...
    return re.sub(r'\\(.)', lambda m: _LUA_ESCAPES.get(m.group(1), m.group(1)), s)
    ...
    def iter_parse(self, path: str, factory: PackageFactory) -> Iterable[PackageMaker]:
        ...
        for xmake_lua_path in walk_tree(packages_root, name='xmake.lua'):
            ...
            try:
                ...
                factory.log(f'cannot read {rel_path}: {e}', severity=Logger.ERROR)
                continue
            ...
            if name_match is None:
                factory.log(f'no package() declaration in {rel_path}, skipping', severity=Logger.ERROR)
                continue
            ...
            if not versions:
                factory.log(f'no add_versions() entries in {rel_path} ({pkgname}), skipping', severity=Logger.ERROR)
                continue
